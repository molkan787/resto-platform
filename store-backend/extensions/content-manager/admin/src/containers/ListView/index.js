...
function ListView({
  ...
  const request = (url, options, ...args) => {
    if(options.method == 'GET'){
      const sch = collectionSchema;
      const needsStoreIdFilter = sch && sch.attributes && typeof sch.attributes.store_id == 'object';
      if(needsStoreIdFilter){
        url += '&store_id=' + window.localStorage.getItem(Config.LS_STORE_ID_KEY);
      }
    }
    return _request(url, options, ...args);
  }
